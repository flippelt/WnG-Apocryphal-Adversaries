...
//   * a duplicate `_id` among a threat's embedded items;
...
// Foundry v14 DocumentIdField is exactly 16 alphanumeric characters. Shorter
// mnemonic ids (the three Heretic Astartes cult-legion folders used 15) throw
// DataModelValidationError and abort the whole compendium. Pad, don't invent
// a new id, when touching a published folder.
...
for (const pack of declared) {
  ...
  for (const file of files) {
    ...
    const isFolder = file.startsWith("_Folder_") || doc._key?.startsWith("!folders!");
    ...
    if (Array.isArray(doc.items)) {
      ...
      for (const item of doc.items) {
        if (typeof item?._id !== "string" || !ID_PATTERN.test(item._id)) {
          fail(where, `embedded item ${JSON.stringify(item?.name ?? "(unnamed)")} has an invalid \`_id\``);
        } else if (embedded.has(item._id)) {
          fail(where, `embedded items reuse \`_id\` ${item._id}`);
        } else {
